...
from . import models_pytensor as models
...
class Experiment(object):
	...
	def test_model(self, model_s):
		...
		test_rows = self.test.indexes[:, 0]
		test_cols = self.test.indexes[:, 1]
		test_tubes = self.test.indexes[:, 2]
		test_ys = self.test.values

		# Compute loss on test set
		test_loss_fn = pytensor.function(
			[model.rows, model.cols, model.tubes, model.ys],
			model.loss
		)
		test_loss = test_loss_fn(test_rows, test_cols, test_tubes, test_ys)
